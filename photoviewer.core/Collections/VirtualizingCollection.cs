...
namespace photoviewer.core.Collections
{
    /// <summary>
    ...
    public class VirtualizingCollection<T> : IList<T>, IList
    {
        ...
        /// </summary>
        public event PropertyChangedEventHandler PropertyLoadingPage;

        /// <summary>
        /// Raises the <see cref="PropertyLoadingPage"/> event.
        /// </summary>
        protected virtual void OnPropertyChanged(object rezult)
        {
            var h = PropertyLoadingPage;
            if (h != null)
                h(rezult, null);
        }
        ...
        public T this[int index]
        {
            get
            {
                ...
                //if(_pages[pageIndex] != null && _pages[pageIndex].Count > 0)
                 //  OnPropertyChanged(_pages[pageIndex]);
        ...
        /// ???????? ??????????? ????????
        /// </summary>
        public List<T> ElemetsLoadingPage { get { return SelectPageData(); } }
        private List<T> SelectPageData()
        {
            if (_pages != null  && _pages.Count == 1) return _pages[0].ToList();
            return new List<T>();
        }
        ...
        {
            var page = await FetchPage(pageIndex);
        ...
        public T TopItem()
        {
            return _pages.ContainsKey(0) ? _pages[0].FirstOrDefault() : default(T);
        }
