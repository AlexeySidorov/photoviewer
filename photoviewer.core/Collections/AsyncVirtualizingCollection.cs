...
namespace photoviewer.core.Collections
{
    ...
    public class AsyncVirtualizingCollection<T> : VirtualizingCollection<T>, INotifyCollectionChanged, INotifyPropertyChanged
    {
        ...
        protected override void LoadCount()
        {
            Count = 0;
            IsLoading = true;
            Task.Factory.StartNew(LoadCountWork);
        ...
        /// </summary>
        private async void LoadCountWork()
        {
            var count = await FetchCount();
            SynchronizationContext.Post(LoadCountCompleted, count);
        }
        ...
        {
            IsLoading = true;
            Task.Factory.StartNew(() => LoadPageWork(index));
        ...
        private async void LoadPageWork(int pageIndex)
        {

            var page = await FetchPage(pageIndex);
            SynchronizationContext.Post(LoadPageCompleted, new object[] { pageIndex, page });
        }
